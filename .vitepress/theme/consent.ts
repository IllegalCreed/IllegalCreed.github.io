export const ANALYTICS_CONSENT_STORAGE_KEY =
  "illegalcreed.analytics-consent.v1";
...
function isStoredConsent(
  value: unknown,
): value is Exclude<AnalyticsConsent, "unset"> {
...
export function readAnalyticsConsent(
  storage: Pick<ConsentStorage, "getItem"> | undefined = browserStorage(),
): AnalyticsConsent {
...
export function writeAnalyticsConsent(
  ...
  storage: Pick<ConsentStorage, "setItem"> | undefined = browserStorage(),
  ...
  try {
    ...
    // The stored choice remains authoritative if CustomEvent is unavailable.
  }
  return true;
}
...
export function subscribeAnalyticsConsent(
  ...
  return () =>
    eventTarget.removeEventListener(ANALYTICS_CONSENT_EVENT, handleConsent);
}
