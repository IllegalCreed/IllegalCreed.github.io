...
export const PERSONAL_SITE_GA_MEASUREMENT_ID = "G-CHX5JE8W8H";
...
function normalizeCampaignToken(
...
export function sanitizePageViewUrl(path: string, origin: string): string {
  ...
  for (const field of UTM_FIELDS) {
    const value = normalizeCampaignToken(
      source.searchParams.get(`utm_${field}`),
    );
...
export function startGoogleAnalytics(
  ...
  const sendPageView = (page: AnalyticsPage) => {
    ...
    try {
      const pageLocation = sanitizePageViewUrl(
        page.path,
      ...
      // Analytics is optional and must not break the documentation site.
    }
  };
