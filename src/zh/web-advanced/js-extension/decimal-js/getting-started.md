...
> 本篇带你从「为什么需要 decimal.js」到「会用基础 API」。版本基线 **decimal.js 10.6.0**。对比对象：同作者的 **big.js**（最精简）、**bignumber.js**（居中、支持多进制）。
...
::: tip 为什么优先用字符串
传 number 时，decimal.js 接收到的已经是 JavaScript 舍入后的浮点值，无法恢复被丢掉的数字；例如 `new Decimal(1.0000000000000001)` 得到 `1`，字符串版本才保留末位。**外部十进制数据优先以字符串传入**，避免在构造前就损失精度。
:::
