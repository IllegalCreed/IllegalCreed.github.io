...
> 版本基线 **decimal.js 10.6.0**。本篇把「会用 API」深化到「懂精度与舍入」：浮点误差的本质、`precision`（有效数字）与 `rounding`（舍入模式）、NaN / Infinity 处理，并理清与 **big.js / bignumber.js** 的核心差异。

## 速查

- JavaScript `number` 是二进制浮点；十进制字符串进入 Decimal 后按十进制语义保存和计算
- `precision` 默认 `20`，控制结果的**有效数字总数**，不是小数点后的位数
- 加、减、乘同样受全局精度约束；除法、开方和非整数幂更容易触发舍入
- `toFixed(dp)` / `toDP(dp)` 按小数位，`toPrecision(sd)` / `toSD(sd)` 按有效数字
- 默认舍入是 `ROUND_HALF_UP`；金融规则需按业务明确选择，不能假定一律使用银行家舍入
- `ROUND_HALF_EVEN` 在精确中点取偶；调用级 `rm` 参数会覆盖全局 `rounding`
- decimal.js 接受 `NaN` / `Infinity`；业务不允许特殊值时必须显式 `isFinite()` 校验
- 轻量四则可选 big.js，多进制 2~36 I/O 可选 bignumber.js，高级数学函数选 decimal.js
...
decimal.js 对外提供**任意精度十进制语义**，内部用「符号 `s` + 指数 `e` + 以 `1e7` 分块的数字数组 `d`」保存系数。十进制字符串不会先落入二进制浮点，因此没有原生 number 的这类「二进制尾巴」。
...
舍入模式可在两处生效：① 全局 `Decimal.set({ rounding })`；② 输出 / 舍入方法的 `rm` 参数（如 `toFixed(2, Decimal.ROUND_DOWN)`、`toDP(2, rm)`）；调用级参数优先于全局。
