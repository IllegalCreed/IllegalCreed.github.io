...
::: tip 本篇范围
本篇聚焦 **crypto-js**——一个用纯 JavaScript 实现的加密算法库。重点在：**哈希（MD5/SHA 系列/历史命名为 SHA3 的 Keccak/RIPEMD160）、HMAC、对称加密（AES/DES/3DES/Rabbit/RC4）、编码器（Base64/Hex/Utf8…）、WordArray 数据结构、密钥派生（PBKDF2/EvpKDF）、加密模式（CBC 默认/CFB/CTR/OFB/ECB）与填充（Pkcs7 默认）、CipherParams 结构、口令模式 vs 直接传 key+iv**。版本基线 **crypto-js 4.2.0**，并贯穿两条安全主线：**MD5/SHA1/DES/RC4 仅用于兼容**，以及 **与原生 Web Crypto API 的取舍**。
:::

crypto-js 是由 **Jeff Mott** 等发起、长期由 **brix** 维护的纯 JS 加密库，官方一句话可概括为「**JavaScript implementations of standard and secure cryptographic algorithms**」——用 JavaScript 实现一批标准加密算法。它最大的特点是 **同步 API、调用极简、算法主体由纯 JavaScript 实现**：`CryptoJS.SHA256("msg")`、`CryptoJS.AES.encrypt("msg", "pass")` 一行就能用。要注意 4.x 已把随机数改为调用环境原生 Crypto；salt / IV 生成时若没有 `getRandomValues` 或 `randomBytes`，会直接抛错而不是降级到 `Math.random()`。
...
- **算法实现纯 JS**：摘要与密码算法不依赖原生算法绑定；4.x 的安全随机数明确委托给平台 Crypto
...
- **旧环境随机数会失败**：4.x 不再回退 `Math.random()`，缺少原生 Crypto 的环境无法生成 salt / IV
...
<a href="https://quiz.illegalscreed.cn/?category=crypto-js" target="_blank" rel="noopener noreferrer">crypto-js 测试题</a>
