...
> PapaParse 5.x 的 API、配置选项、结果对象与常量速查。两个核心方法：`Papa.parse(input, config)` 与 `Papa.unparse(data, config)`。
...
- **`meta`** 常见字段：`delimiter`、`linebreak`、`aborted`、`truncated`、`cursor`，以及 `header:true` 时的 `fields`、`renamedHeaders`。
