...
> 本篇讲 **ts-pattern 的基础用法**：`match`/`with`/`exhaustive`/`otherwise`、字面量与对象模式、最常用的 `P.*` 通配模式。版本基线 **ts-pattern 5.9.0**。对比对象：原生 `if/else`、`switch`。
...
> 边界提醒：`isMatching(pattern, unknownValue)` 可以做布尔式结构校验与类型收窄，但 ts-pattern 不提供结构化错误、转换或默认值，不等同于完整 schema 库。复杂入口通常先由 Zod/Valibot 解析，再交给 ts-pattern 做穷尽分发。
