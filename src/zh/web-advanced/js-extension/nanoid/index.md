...
::: tip 本篇范围
本篇聚焦 **nanoid**——一个极小、安全、URL 友好的唯一字符串 ID 生成器。重点在：**默认 21 字符 + 64 个 URL 安全字母表**的设计、用**加密随机源**（`crypto` / Web Crypto）而非 `Math.random()`、`nanoid(size)` 自定义长度、`customAlphabet` 自定义字母表、`customRandom` 替换随机源、`nanoid/non-secure` 非加密版、碰撞概率与长度/字母表的权衡，以及与 **UUID（v4 / v7）** 的体积/速度/可读性对比。版本基线 **nanoid 5.1.16**（ESM-only 产物），并在关键处点明 **3.x ↔ 5.x 差异**。
:::
...
理解 nanoid 的关键是它的**熵模型**：ID 的随机性 ≈ `log2(字母表大小) × 长度`。默认用 64 个符号、长度 21，约携带 **126 位随机性**，与 UUID v4 的 122 位相当——也就是说它在更短的字符串里塞进了相近的抗碰撞能力。**2026 年的现状**：当前发布版为 **5.1.16**，只发布 ESM 产物；早期的 `nanoid/async`、`nanoid/url-alphabet` 子入口已移除（`urlAlphabet` 改从主入口具名导出）。CommonJS 的选择取决于 Node：22.12+ 可直接加载 ESM，Node 20 需实验标志，Node 18 可动态 import；旧工具链也可使用官方仍支持的 3.x。
...
- **ESM-only 仍有迁移摩擦**：能否从 CommonJS 同步加载取决于 Node 版本；Jest 等旧 CJS 工具链也可能需要 ESM 配置、转译或 3.x
...
<a href="https://quiz.illegalscreed.cn/?category=nano-id" target="_blank" rel="noopener noreferrer">Nano ID 测试题</a>
