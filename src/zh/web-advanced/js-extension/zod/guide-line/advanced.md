...
> 版本基线 **Zod 4**。把 Zod 用进真实项目：`refine`/`superRefine` 跨字段校验、`transform`/`pipe` 流水、判别联合、递归 schema、错误处理，以及 tRPC / React Hook Form / env 校验生态接入。
...
const Tags = z.array(z.string()).superRefine((val, ctx) => {
  if (val.length > 3) {
    ctx.addIssue({ code: "too_big", maximum: 3, inclusive: true, message: "最多 3 个" });
  }
...
```ts
// 字符串 → 转数字 → 断言非负
const NonNegFromString = z.string()
...
进入 [指南 · 专家](./guide-line/expert)：v3 → v4 完整迁移、Zod Mini 与 tree-shaking、`z.toJSONSchema` 与元数据注册表、品牌类型、全局错误与 i18n、库作者最佳实践。
