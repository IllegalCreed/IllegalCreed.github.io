...
::: tip 本篇范围
本篇聚焦 **Immer —— 用「可变写法」安全产出不可变（immutable）数据的微型库**。它与手写不可变更新（层层 `...` 展开）、`structuredClone` 同属「不可变状态更新」选型方向，本篇在取舍与对比时一笔带过另两者。版本基线 **Immer 11.1.11**（v11 重写终态化系统、默认改为 loose iteration，并加入 array methods 插件）。
:::
...
- **Proxy 开销**：官方旧基准中的一个最坏场景约为手写 reducer 的 2–3 倍，并非所有负载的固定结论；超热路径应实测后取舍
...
<a href="https://quiz.illegalscreed.cn/?category=immer" target="_blank" rel="noopener noreferrer">Immer 测试题</a>
