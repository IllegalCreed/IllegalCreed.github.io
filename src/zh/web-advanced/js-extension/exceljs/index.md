...
::: tip 本篇范围
本篇聚焦 **ExcelJS —— 一个在 Node.js 与浏览器中读取、操作、写入 XLSX 电子表格数据与样式的 JavaScript 库**。它与 SheetJS（xlsx）同属「电子表格读写」选型方向，本篇在对比与定位时一笔带过另一者。版本基线 **ExcelJS 4.x**（npm latest 为 `4.4.0`，MIT 许可）。
:::
...
- **流式读写省内存**：`stream.xlsx.WorkbookWriter` / `WorkbookReader` 配 `commit()` 与 `for await`，可处理几十万行而不 OOM
