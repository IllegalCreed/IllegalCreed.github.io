...
> 版本基线 **Fuse.js 7.x**。把 Fuse.js 用进真实项目：`ignoreLocation` 与位置调参、扩展搜索语法（`'`/`!`/`^`/`$`/`=`）、逻辑搜索（`$and`/`$or`/`$path`/`$val`）、动态增删数据、其它常用选项默认值。
...
> 改某一条：先 `removeAt` 再 `add`（没有 `update`）。**切勿**对实例 `push`/`splice`——数组方法不会更新索引。
