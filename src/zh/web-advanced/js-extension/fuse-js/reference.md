...
| 产物 | 体积（min+gzip） | 含 |
|---|---|---|
| 完整版（`fuse.mjs` / `fuse.cjs`） | 约 8.6 KB | 全部特性 |
| basic 版（`fuse.basic.min.mjs`） | 约 6.8 KB | 仅核心模糊搜索 |
| worker 版（`fuse.worker.mjs`） | — | 配合 Web Worker |

---

API 查完，回 [指南 · 基础](./guide-line/base) 看 keys 与调参，或 [指南 · 专家](./guide-line/expert) 看算法与工程化。
