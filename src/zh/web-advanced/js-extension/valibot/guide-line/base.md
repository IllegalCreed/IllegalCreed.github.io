...
> 版本基线 **Valibot 1.4.2**。本篇把「会装会用」推进到「懂构件与语义」：schema 全景、`pipe()` 与 action（校验 vs 转换）、可选/可空与默认值、对象额外键策略、类型推导。

## 速查

- 基线：Valibot `1.4.2`，零依赖、ESM / CommonJS 双入口、构建目标 ES2020；TypeScript 最低 `5.0.2`
- 心智模型：schema 定义类型，action 进入 pipeline，method 以 schema 或 action 为首参进行解析、派生或包装
- `v.pipe(schema, ...items)` 从左到右执行，schema 后最多再接 19 个 schema / action；更长流程可嵌套 pipe
- 字面量集合用 `v.picklist([...])`；TypeScript enum 对象用 `v.enum(E)`，具名导入名为 `enum_`
- 可选包装：`optional` 只接受 `undefined`、`nullable` 只接受 `null`、`nullish` 同时接受两者
- 对象缺失的 optional 键默认不执行其 pipe；提供默认值后 pipeline 才会运行且输出键变为必有
- 对象额外键：`object` 剔除、`strictObject` 报错、`looseObject` 保留、`objectWithRest` 校验其余值
- 类型：日常读取 `InferOutput`；transform、默认值、brand、readonly 等场景再区分 `InferInput`
...
// 第二参 = 默认值（optional 在缺省/undefined 时补齐）
