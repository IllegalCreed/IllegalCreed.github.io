...
> 本篇带你从零理解 **RxJS** 的核心三件套与第一条数据流。版本基线 **RxJS 7.x**（操作符已扁平导出到 `'rxjs'`，`toPromise` 已废弃）。对比对象：Promise / async-await、EventEmitter。
...
- 取消：`sub.unsubscribe()` —— ⚠️ 持续型流忘记退订会**内存泄漏**
...
| 可取消 | — | 不可取消 | **可取消**（unsubscribe） |

> 一句话：Observable = 「**多值 + 可取消 + 惰性**」的 Push 流。这正是它相对 Promise 的核心价值。
...
- **Subscription**：一次「正在进行的执行」的句柄，`unsubscribe()` 取消它。
...
> ⚠️ 空流（不发值就 complete）时，`firstValueFrom`/`lastValueFrom` 会 **reject 一个 `EmptyError`**；可传 `{ defaultValue }` 兜底。这是与旧 `toPromise()`（空流 resolve `undefined`）的关键差异。
...
**核心认知**：活跃订阅不会被垃圾回收。持续型流（`interval`、`fromEvent`、长生命周期 `Subject`）必须显式收尾——手动 `unsubscribe`、`takeUntil(destroy$)`、`take/first` 自动收尾，或框架层 `async` 管道。
