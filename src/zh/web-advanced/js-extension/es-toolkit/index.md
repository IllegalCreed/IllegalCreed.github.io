...
::: tip 本篇范围
本篇聚焦 **es-toolkit**——2024 年崛起的现代 JavaScript 工具函数库，定位为 **Lodash 的高性能、更小体积替代**。重点在：相比 lodash 的体积 / 性能优势（用官方数字）、主包 `es-toolkit` 与兼容层 `es-toolkit/compat` 的分工、tree-shaking 与 ESM/CJS 双格式、TS-first 的现代 API 设计，以及从 lodash / lodash-es 的迁移路径与取舍。版本基线 **es-toolkit 1.49.0**。
:::

es-toolkit 是由韩国金融科技公司 **Toss** 开源的现代 JavaScript 工具库，官方一句话定位是「**a modern JavaScript utility library that offers a collection of powerful functions for everyday use**」。它覆盖数组、函数、数学、对象、断言（predicate）、Promise、字符串等日常领域，主打相比 lodash **体积最多小约 97%**、官方性能总括**平均约 2 倍**、**内置 TypeScript 类型**且 **100% 测试覆盖**。具体函数并非一律更快，选型时应读取基准口径。它已被 **Storybook、MUI、Recharts** 等成熟项目用于生产。
...
**主包 vs 兼容层**是理解 es-toolkit 的关键：主包 `es-toolkit` 只暴露**类型安全的现代形态**；`es-toolkit/compat` 则复刻大量 Lodash 函数签名与边界，专供存量项目渐进迁移。官方称 compat 自 v1.39.3 起通过 Lodash 测试套件，但同一份设计说明也把方法链、跨 realm、修改内建原型、部分隐式转换与特化函数列为范围外。因此「先改 import」是降低改造量的起点，不是免测试承诺。**新项目应直接用主包**；compat 只为迁移而生。
...
- **官方性能总括平均约 2 倍**：当前明细基准里 `pick` 约 3.9×、`omit` 约 3.3×，但 `union` 与 `groupBy` 也存在低于 lodash 的样本
...
- **平滑迁移**：`es-toolkit/compat` 覆盖大量 Lodash API，可先缩小调用点改造范围，再逐步切主包

**缺点**

- **主包不是 lodash 全集**：有意只提供现代核心函数；深路径 `get/set` 等迁移能力在 compat，而 Lodash wrapper 方法链在 compat 也不支持
- **compat 有开销**：为复刻 Lodash 签名与边界背负额外逻辑，比主包略大、略慢；主包基准不能直接外推到 compat
...
- **仍有明确范围外行为**：即便 compat 也不支持 wrapper 方法链、跨 realm / 修改原型等情形，`sortedUniq`、`sortedUniqBy`、`mixin`、`noConflict`、`runInContext` 也不在当前导出中
