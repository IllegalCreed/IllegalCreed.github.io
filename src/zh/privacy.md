...
本政策适用于个人网站 `illegalscreed.cn` 和算法可视化站
`algo.illegalscreed.cn`。

## 处理的信息

个人网站与算法可视化站分别使用独立的 Google Analytics 4 属性了解汇总页面访问
情况。只有在你于站内统计提示中选择“允许”后，站点才会加载 Google Analytics
脚本。允许后，Google Analytics 可能根据 Google 的政策处理经过清洗的页面 URL、
浏览器和设备特征、大致位置、来源页面，并使用 Cookie 或类似标识符。

两个站点只发送标准页面浏览事件。发送给 Analytics 的页面 URL 会移除任意查询参数
和 hash；只有经过校验的 `utm_source`、`utm_medium`、`utm_campaign` 和
`utm_content` 渠道参数可以保留。搜索词、算法输入、播放、测验、分享及其他交互
不会作为自定义 Analytics 事件发送。
...
请求路径、响应状态和时间。这两个站点当前不提供账号注册或评论功能。

## 广告选择与同意

你可以在两个站点上接受、拒绝或之后重新打开“隐私设置”；拒绝时不会加载 Google
Analytics。你也可以在 [Google 广告设置](https://adssettings.google.com/)中
查看或修改广告个性化设置，或安装
