...
- [入门](./getting-started) —— micro-app 解决什么、最小接入（`<micro-app name url>` 一行标签）、CustomElement 容器心智、与 qiankun/wujie 的接入成本对比
- [CustomElement 容器](./guide-line/custom-element) —— 借 WebComponent 的 CustomElement 定义 `<micro-app>` 标签、`name`/`url`/`baseroute` 等属性、组件化接入、生命周期事件（created/beforemount/mounted/unmount/error）
