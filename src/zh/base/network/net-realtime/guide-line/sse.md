...
SSE 最实用的内建能力是**自动重连**：当连接因网络抖动、服务器重启或代理超时而中断，浏览器会在等待一段时间（默认值由浏览器决定，可被 `retry:` 字段覆盖）后**自动重新发起请求**，无需任何客户端代码。容易被忽略的是：按 WHATWG 规范，**服务器正常结束响应同样会触发自动重连**——重连并不区分"异常断"与"正常完"；想让浏览器彻底停下，要么服务端返回 **HTTP 204**，要么客户端主动 `close()`（API 细节见 [Web API 章 SSE 叶](/zh/web-advanced/web-api/sse/)）。
