...
- **输出灵活**：`writeFile`（Node 写盘 / 浏览器下载自适应）、`write`（base64/blob/nodebuffer 等数据形态）、`stream`（Node 下返回 Buffer；名称不代表 Readable）
