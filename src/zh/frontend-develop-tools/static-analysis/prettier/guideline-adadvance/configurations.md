...
- 检测忽略 Pragma 注释：`checkIgnorePragma: false`（v3.6.0+）
...
- 使用 TS 配置文件：Node ≥ 22.6.0 支持（22.6–24.2 需 `--experimental-strip-types`，≥ 24.3.0 原生支持）
...
### 检测忽略 Pragma 注释 <Badge type="tip" text="v3.6.0+" />

控制 Prettier 是否识别文件顶部的 "不要格式化我" pragma 注释，将其视为忽略文件。

**有效值：**

- `true`：当文件顶部带有 `@noprettier` 或 `@noformat` pragma 注释时，跳过该文件的格式化
- `false`：忽略此类 pragma，按常规规则处理
...
// 启用 checkIgnorePragma 后，本文件会被跳过
const messy   =    { a:1,b:2 }
```

**使用场景**：对自动生成或风格特殊的少量文件，相比维护 `.prettierignore`，直接在文件头加 pragma 更内聚。
...
TypeScript 配置文件（`.prettierrc.ts`, `prettier.config.ts` 等）需要 Node.js 支持。版本要求：

- **Node.js < 22.6.0**：不支持，只能使用 JSDoc 在 `.js` 配置文件中加类型信息。
- **22.6.0 ≤ Node.js < 24.3.0**：需要 `--experimental-strip-types` 标志启用原生 TS 执行。
- **Node.js ≥ 24.3.0**：原生支持执行 `.ts` 文件，不再需要 `--experimental-strip-types`。
