...
- **TypeScript**：`typescriptlang.org/play`，可切编译器版本与 flags，并查看 JS、声明文件、AST 等编译结果。
- **Vue**：`play.vuejs.org`，围绕 `.vue` SFC、Import Map、SSR/编译输出与 Vue 版本切换组织多文件示例。
- **Svelte**：`svelte.dev/playground`，原 `/repl/` 已迁移；保存后生成 `/playground/{id}` 形式的可分享示例。
- **分享机制不同**：TS 主要把代码放 hash、配置放 query；Vue 序列化完整状态到 hash；Svelte 以保存后的短链为主。
- **多文件机制不同**：TS 用 `// @filename:` 虚拟文件，Vue/Svelte 使用可见文件标签，不能把三者的导入规则混为一谈。
- **看编译产物**：Playground 的核心优势是暴露框架/编译器输出；需要终端、服务端或完整依赖安装时转向通用在线编辑器。
- **复现问题时固定版本**：分享 URL 时同时保留编译器/框架版本，避免 `next/dev` 更新后示例行为漂移。
