...
- **主入口**：`https://snack.expo.dev/`；已存项目可用 `/{id}` 或 `/@用户名/slug`，旧 `snack.expo.io` 已跳转。
- **嵌入方式**：`embed.js` 使用 `data-snack-*` 属性；属性值需 URL 编码，避免代码、依赖或 JSON 被截断。
- **ID 优先级最高**：提供 `data-snack-id` 后，内联 `code` 与 `dependencies` 会被忽略。
- **默认预览**：`data-snack-platform` 默认 `web`；可选 `ios/android/web/mydevice`，真机预览依赖 Expo Go。
- **多文件**：URL 的 `files` 参数承载 JSON 文件映射；大型示例应优先保存 Snack 后分享 ID，而不是无限膨胀 URL。
- **依赖版本**：省略版本时按所选 Expo SDK 解析兼容版本，原生模块能否运行仍受 Expo Go/SDK 能力边界约束。
- **适用范围**：Snack 适合 Expo/React Native 教学与复现，不是任意原生工程、私有原生模块或完整 CI 环境。
