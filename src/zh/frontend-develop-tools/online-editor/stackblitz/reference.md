...
- **项目 URL**：`/edit/<id>` 打开、`/fork/<id>` 复制；GitHub 仓库走 `/github/{owner}/{repo}`，Codeflow 使用 `pr.new`。
- **速建短链**：`vite.new`、`node.new`、`nextjs.new`、`nuxt.new` 等由框架模板直接创建项目。
- **嵌入 SDK**：`@stackblitz/sdk` 通过 `embedProject*` 嵌入，通过 `openProject*` 新窗口打开；URL 参数大小写敏感。
- **WebContainers**：浏览器内运行 Node.js 工具链，核心流程是 `boot()` → `mount()` → `spawn()` → 监听 `server-ready`。
- **安全边界**：WebContainers 依赖跨源隔离与浏览器能力；服务端密钥、真实云资源和不兼容的原生二进制不应放入前端沙箱。
- **项目配置**：`.stackblitzrc` 只承载官方支持的项目启动配置，不要把任意应用配置都塞进去。
- **选 VM 的时机**：需要完整 Linux、持久后台服务或原生依赖时，使用远程 VM 类编辑器，而不是强行绕过 WebContainer 限制。
