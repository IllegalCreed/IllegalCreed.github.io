...
- **先分两代产品**：Classic Pen 是 HTML/CSS/JS 三面板；CodePen 2.0 是多文件 Pen，文件上限、相对路径和弃用项不能混写。
- **现行预处理器标识**：HTML 用 `html/markdown/pug`，CSS 用 `css/scss/sass/less/stylus/postcss`，JS 用 `js/babel/typescript`。
- **Prefill Embed**：外层 `data-prefill` 放元数据，代码放 `<pre data-lang>`；JSON 与 HTML 特殊字符都要正确转义。
- **嵌入脚本分流**：经典已有 Pen 用 `cpwebassets.codepen.io/assets/embed/ei.js`；Prefill/新嵌入用 `public.codepenassets.com/embed/index.js`。
- **URL Extensions**：Pen URL 可追加 `.css`、`.js`、`.html` 取对应面板；2.0 多文件用 `?file=/path` 指定文件。
- **大小口径**：Classic 约 1MB 是整 Pen 总量；2.0 的 1MB 是单个文本文件，媒体与文件数按 PRO 等级变化。
- **已废弃项**：Haml、Slim、CoffeeScript、LiveScript、Flutter、Professor Mode 不应再作为 2.0 现行能力介绍。
